...
export default class Autowire {
  ...
  * _walkSync (dir) {
    ...
    for (const file of files) {
      ...
      const { matchExcluded, filePath } = this._matchExcluded(dir, file)
      if (!matchExcluded) {
        yield filePath
      }
    }
   ...
   * @returns {any}
   */
  _matchExcluded(dir, file) {
    const filePath = path.join(dir, file.name)
    const matchExcluded = this._excludedFolders.find(excludedFolder => {
      return filePath.includes(excludedFolder)
    })
    return { matchExcluded, filePath }
  }
