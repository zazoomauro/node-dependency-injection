...
class FileLoader {
  ...
  _parseTags (definition, tags = []) {
    for (let tag of tags) {
      definition.addTag(tag.name, this._parseTagAttributes(tag.attributes))
    }
  }
   ...
   * @private
   */
  _parseTagAttributes (attributes) {
    let map = new Map()
  ...
  _requireClassNameFromPath (classObject) {
    let classPath
    if (classObject.includes('%') && classObject.slice(-1) === '%') {
      classPath = classObject.slice(1)
    } else {
      let fromDirectory = path.dirname(this.filePath)
      classPath = path.join(fromDirectory, classObject)
    }
