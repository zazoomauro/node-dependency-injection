...
export default class InstanceManager {
  ...
  _getInstanceFromStringId (id, bypassPublic) {
    ...
    if (this._definitions.has(id)) {
      this._containerBuilder.loggerHelper.debug(`Resolving service: ${id}`)
      return this._getInstanceFromId(id, bypassPublic)
    }

    this._containerBuilder.loggerHelper.warn(`The service ${id} is not registered`)
  ...
  _getInstanceFromFunction (functionName, bypassPublic) {
    const ids = this._containerBuilder.objectToId.get(functionName)
    if (ids && ids.size > 0) {
      return this.getInstance(ids.values().next().value, bypassPublic)
    }
    this._containerBuilder.loggerHelper.warn(
  ...
  _logDeprecations (definition) {
    if (definition.deprecated) {
      this._containerBuilder.loggerHelper.warn('DEPRECATED', definition.deprecated)
    }
  }
  ...
  _resolveServices (value) {
    ...
    if (value instanceof ParameterReference) {
      return this._containerBuilder.getParameter(value.key)
    }

    if (value instanceof TaggedReference) {
      return this._findPriorityTaggedServices(value.tag, value.indexAttribute)
    }
    ...
    if (value instanceof KeyedReference) {
      return this._containerBuilder.getKeyed(value.group, value.key)
    }

    if (value instanceof KeyedGroupReference) {
      return this._containerBuilder.getKeyedGroup(value.group)
    }
  ...
  /**
   * Legacy tagged collection resolution used by !tagged.
   * Preserves definition order and intentionally ignores tag attributes.
   *
  ...
  /**
   * Symfony-style tagged collection resolution used by @tagged(...).
   * Tag membership is plural; the optional priority attribute orders members,
   * and an optional index attribute projects the collection as a keyed Map.
   * Services with equal priority retain definition order.
   *
   * @param {string} tagName
   ...
   * @private
   */
  _findPriorityTaggedServices (tagName, indexAttribute = null) {
    const candidates = []
    let sequence = 0

    for (const [id, definition] of this._definitions) {
      const tags = definition.tags.filter((tag) => tag.name === tagName)
      if (tags.length === 0) {
        continue
      }

      if (indexAttribute === null) {
        const priority = this._tagPriority(tags[0], tagName, id)
        candidates.push({ id, priority, index: null, sequence: sequence++ })
        continue
      }

      const indexes = new Set()
      for (const tag of tags) {
        const priority = this._tagPriority(tag, tagName, id)
        const index = tag.attributes.has(indexAttribute)
          ? tag.attributes.get(indexAttribute)
          : id
        ...
        indexes.add(index)
        candidates.push({ id, priority, index, sequence: sequence++ })
      }
    }
    ...
    if (indexAttribute === null) {
      return candidates.map(({ id }) => this.getInstance(id, true))
    }

    const result = new Map()
    for (const { id, index } of candidates) {
      result.set(index, this.getInstance(id, true))
    }
    return result
  }

  /**
   * @param {{attributes: Map}} tag
   * @param {string} tagName
   * @param {string} serviceId
   * @returns {number}
   * @private
   */
  _tagPriority (tag, tagName, serviceId) {
    const rawPriority = tag.attributes.has('priority')
      ? tag.attributes.get('priority')
      : 0
    const priority = typeof rawPriority === 'string' && /^-?\d+$/.test(rawPriority)
      ? Number(rawPriority)
      : rawPriority

    if (!Number.isInteger(priority)) {
      throw new TypeError(
        `Tag '${tagName}' on service '${serviceId}' has non-integer priority '${rawPriority}'`
      )
    }
  ...
  searchDefinitionsToOverrideArgs () {
    ...
    for (const [key, definition] of this._definitions) {
      if (definition.overrideArgs.length > 0) {
  ...
  searchNotOverrideDefinitionsByObject (Object) {
    const result = new Map()
    for (const [key, definition] of this._definitions) {
      if (definition.Object?.name === Object.name && definition.overrideArgs.length === 0) {
        result.set(key, definition)
      }
    }
    return result
