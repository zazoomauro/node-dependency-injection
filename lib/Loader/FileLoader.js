...
class FileLoader {
  ...
  _tryToGuessClassPath(classObject) {
    ...
    let paths = require.resolve.paths(path.join(fromDirectory, classObject)) || [];
    let dir = path.dirname(classObject);
    paths.push(path.join(fromDirectory, dir));
    paths.push(fromDirectory);
    if (this.container.defaultDir) {
      paths = paths.concat(require.resolve.paths(path.join(this.container.defaultDir, classObject)) || [])
      paths.push(path.join(this.container.defaultDir, dir));
    }
    if (!isRelativePath) {
      paths = paths.concat(require.resolve.paths(classObject) || []);
    }
    if (this.container.defaultDir) {
      fromDirectory = isRelativePath ? path.join(this.container.defaultDir, classObject) : this.container.defaultDir;
      let dirs = fromDirectory.split(path.sep);
      for (let i = dirs.length; i > 1; ++i) {
        let dir = '/' + path.join(...dirs.slice(0, i+1), 'node_modules');
        paths.push(dir);
      }
    }
    
    if (this.container.defaultDir) {
      paths.push(this.container.defaultDir);
    }
    paths = [...new Set(paths)];
