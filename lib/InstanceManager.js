...
import SetterParamNotFoundException
  from './Exception/SetterParamNotFoundException'
...
export default class InstanceManager {
  ...
  _getNotSyntheticInstanceFromDefinition (definition) {
    ...
    for (const setter of definition.setters) {
      this._setParam(instance, setter)
    }
  ...
  /**
   * @param {*} service
   * @param {{param, argument}} setter
   *
   * @private
   */
  _setParam (service, setter) {
    const descriptor = Object.getOwnPropertyDescriptor(
      service.constructor.prototype, setter.param)
    if (!descriptor || !descriptor.set) {
      throw new SetterParamNotFoundException()
    }

    service[setter.param] = this._resolveServices(setter.argument)
  }
