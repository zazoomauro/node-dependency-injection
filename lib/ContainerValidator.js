...
export default class ContainerValidator {
  ...
  validate () {
    ...
    this._checkKeyedGroupsWithMultipleDefaults(result)
    this._checkInvalidAliases(result)
  ...
  _checkOrphanTaggedServices (result) {
    ...
    for (const definition of this._container.definitions.values()) {
      ...
      for (const arg of allArgs) {
        ...
        if (arg instanceof TaggedReference) {
          usedTags.add(arg.tag)
        }
      }
    }
    ...
    for (const [tagName] of definedTags) {
      if (!usedTags.has(tagName)) {
        result.addInfo(
          ...
          '',
          `tag:${tagName}`,
          `Tag '${tagName}' is defined on services but no consumer uses it via a tagged reference`
        )
      }
    }
  }
  ...
  /**
   * Keyed groups are plural and do not require a default. When a default is
   * declared, however, it must identify exactly one service so singular
   * getKeyed(group) resolution remains unambiguous.
   *
   * @param {ValidationResult} result
   * @private
   */
  _checkKeyedGroupsWithMultipleDefaults (result) {
    ...
    for (const [id, definition] of this._container.definitions) {
      ...
      groups.get(definition.keyedGroup).push({ id, definition })
    }

    for (const [group, services] of groups) {
      const defaults = services
        .filter(({ definition }) => definition.keyedDefault)
        .map(({ id }) => id)
      ...
      if (defaults.length > 1) {
        result.addError(
          ...
          '',
          `keyed:${group}`,
          `Keyed group '${group}' declares multiple defaults: '${defaults.join("', '")}'`
        )
      }
    }
  }

  // ─── Check 8: Invalid aliases ──────────────────────────────────────────────

  /**
   * Runtime alias lookup resolves one alias hop and then requires the target
   * to be a registered definition. Validate that same invariant here so
   * orphaned aliases can be diagnosed even when nothing currently references
   * them.
   *
   * @param {ValidationResult} result
   * @private
   */
  _checkInvalidAliases (result) {
    const aliases = this._container._alias
    if (!aliases) {
      return
    }

    for (const [aliasId, targetId] of aliases) {
      const targetExists = this._container.definitions.has(targetId) ||
        (targetId === 'service_container' && this._container.containerReferenceAsService)
      ...
      if (!targetExists) {
        result.addError(
          ...
          aliasId,
          `@${targetId}`,
          `Alias '${aliasId}' points to undefined service '@${targetId}'`
        )
      }
    }
  }
