...
export default class Autowire {
  ...
  * _walkFilePath (dir, fileName) {
  ...
  /**
   * The strategy used to generate service IDs during autowiring.
   *
   * - `'legacy'` (default): encodes the absolute file path in Base64.
   *   Stable across environments only when paths do not change.
   * - `'readable'`: derives a stable, human-readable ID from the path
   *   relative to `defaultDir` (e.g. `src/Service/Mailer.ts` →
   *   `Service/Mailer`). A legacy-format alias is also registered for
   *   backward compatibility.
   *
   * @param {'legacy'|'readable'} value
   */
  set idStrategy (value) {
    if (value !== 'legacy' && value !== 'readable') {
      throw new Error(`Invalid autowire ID strategy "${value}". Must be "legacy" or "readable".`)
    }
