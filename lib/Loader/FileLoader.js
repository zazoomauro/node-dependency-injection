...
class FileLoader {
  ...
  async _parseDefaults (defaults = {}) {
    ...
    if (defaults.autowireAliasResolution !== undefined) {
      autowire.autowireAliasResolution = defaults.autowireAliasResolution
    }
    ...
    if (defaults.dumpFile) {
      let dumpFilePath = defaults.dumpFile
      if (!path.isAbsolute(dumpFilePath)) {
        const filePathParsed = path.parse(this.filePath)
        dumpFilePath = path.join(filePathParsed.dir, dumpFilePath)
      }
      autowire.serviceFile = new ServiceFile(dumpFilePath, true)
    }
