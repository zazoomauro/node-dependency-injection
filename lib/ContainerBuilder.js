...
class ContainerBuilder {
  ...
  constructor (containerReferenceAsService = false, defaultDir = null) {
    ...
    this._verbosity = 'warn'
    this._loggerHelper = new LoggerHelper(this._logger, this._verbosity)
    this._instanceManager = undefined
    this._containerReferenceAsService = containerReferenceAsService
    this._defaultDir = defaultDir
    this._autowire = null
    this._binds = new Map()
    this._keyedGroups = new Map()
  }
  ...
  /**
   * @param {'silent'|'warn'|'info'|'debug'} value
   */
  set verbosity (value) {
   ...
   * @returns {Definition}
   */
  registerKeyed (group, key, object = null, args = []) {
    const id = `${group}.${key}`
    const definition = this.register(id, object, args)
    definition.keyedGroup = group
    definition.keyedKey = key
    this._addToKeyedGroup(group, key, id)
    return definition
  }
   ...
   * @param {string} id
   */
  _addToKeyedGroup (group, key, id) {
    if (!this._keyedGroups.has(group)) {
      this._keyedGroups.set(group, new Map())
    }
    this._keyedGroups.get(group).set(key, id)
  }
   ...
   * @returns {*}
   */
  getKeyed (group, key = null) {
    const groupMap = this._keyedGroups.get(group)
    if (!groupMap) {
      throw new KeyedGroupNotFoundException(group)
    }

    if (key !== null) {
      const id = groupMap.get(key)
      if (!id) {
        throw new KeyedServiceNotFoundException(group, key)
      }
      return this.get(id)
    }
  ...
  getKeyedGroup (group) {
    ...
    for (const [key, id] of groupMap) {
      result.set(key, this.get(id))
    }
    return result
  }
  ...
  async compile (options = {}) {
    ...
    let shouldValidateAfterCompile = false

    if (options.validate === true) {
      const validator = new ContainerValidator(this)
      validationResult = validator.validate()

      if (!validationResult.isValid) {
        if (options.throwOnError !== false) {
          throw new ContainerValidationError(validationResult)
        }
        return validationResult
      }

      shouldValidateAfterCompile = true
    }

    await new Compiler(this).run()

    if (shouldValidateAfterCompile) {
      const validator = new ContainerValidator(this)
      validationResult = validator.validate()

      if (!validationResult.isValid && options.throwOnError !== false) {
        throw new ContainerValidationError(validationResult)
      }
    }
   ...
   */
  exportGraph (format = 'mermaid', options = {}) {
    return new GraphExporter(this).export(format, options)
  }
  ...
  setDefinition (id, definition) {
    if (definition instanceof Definition) {
      ...
      if (definition.Object) {
        if (!this._objectToId.has(definition.Object)) {
          this._objectToId.set(definition.Object, new Set())
        }
        this._objectToId.get(definition.Object).add(id)
      }
  ...
  * findTaggedServiceIds (name) {
  ...
  removeDefinition (key) {
    if (this._definitions.has(key)) {
      ...
      if (definition.Object) {
        const ids = this._objectToId.get(definition.Object)
        if (ids) {
          ids.delete(key)
          if (ids.size === 0) {
            this._objectToId.delete(definition.Object)
          }
        }
      }
